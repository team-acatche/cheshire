...
def delete_session(
    ...
    if not user_db_path.exists() or not session_dir.exists():
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Session not found")
    with sqlite3.connect(user_db_path) as db:
        SqliteSessionRepository(db).delete_session(session_id)
        SqliteEventRepository(db).delete_messages_from_session(session_id)
    _session_token_buffers.pop(session_id, None)
    task = _session_agent_tasks.pop(session_id, None)
    if task and not task.done():
        task.cancel()
