...
async def evaluate_document(
    ...
    _session_id: str = body.session_id or str(uuid.uuid4())
    filename: str = re.sub(r"[^a-zA-Z0-9_\-\.]", "_", body.uploaded_document.filename or "upload")
    document_path = Path(SESSION_DIR) / body.username / _session_id / "documents" / f"{datetime.now().isoformat()}__{filename}"

    session_path = Path(SESSION_DIR) / body.username / _session_id
    ...
    async with aiofiles.open(document_path, "wb") as d:
        await d.write(await body.uploaded_document.read())
    ...
    if results := await evaluate_file(Path(document_path), config):
        ...
        with sqlite3.connect(session_path / "history.sqlite") as history_db:
            ...
            summary = "\n".join([f"- {v.title}: {v.description}" for v in results])
